...
def monte_carlo1(graph,seed,k,num_simu=10000):
	random.seed()
	inf=0.0
	i_size=0
	r_size=0
		...
		while(i_size>0):
			...
			for i in range(0,i_size):
				...
				for node in graph.graph.successors(i_arr[i]):
					if state[node]==S_STATE:
						pp=float(graph.graph[i_arr[i]][node]['weight'])
						#pp = random.random()
						rand=random.random()
						if rand<pp:
							state[node]=SI_STATE
...
def monte_carlo2(graph,seed,k,num_simu=10000):
	random.seed()
	inf=0.0
	i_size=0
	r_size=0
...
#初始化种子集
	...
	while(len(S)):
		...
		for i in S:
			for node in graph.graph.successors(i_arr[i]):
				if state[node]==S_STATE:
					pp = sum(float(graph.graph[pre][node]['weight']) for pre in set(graph.graph.predecessors(node)).intersection(set(S)))
					rand = 0.5
					if rand<pp:
						state[node]=SI_STATE
						si_arr[si_size]=node
...
def monte_carlo3(graph,seed,k):

	state=[S_STATE]*(graph.num_node)
	S =[]
	for i in range(0, k):
		S.append(seed[i])
		state[seed[i]] = I_STATE

	for node1 in S:
		for node in graph.graph.successors(node1):
			if state[node] == S_STATE:
				pp = sum(float(graph.graph[pre][node]['weight']) for pre in set(graph.graph.predecessors(node)).intersection(set(S)))
				rand = 0.5
				if rand <= pp:
...
def monte_carlo4(graph,seed,k,num):
	...
	for node1 in S:
		for node in graph.graph.neighbors(node1):
			if state[node] == S_STATE:
				pp = (1.0/graph.graph.degree(node))* len(set(graph.graph.neighbors(node)).intersection(set(S)))
				rand = 0.5
				if rand <= pp:
					state[node] = SI_STATE
					S.append(node)
