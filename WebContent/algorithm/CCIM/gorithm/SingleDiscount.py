...
class Single_Discount():
    ...
    def seed_selection(self,k):
        ...
        for i in range(0,k):
            ...
            max_degree = sorted(dict(degree_dict).items(),key=lambda item:item[1],reverse=True)[0]
...
if __name__ == '__main__':
    SD = Single_Discount('../LFR/100_3_0.01.txt')
    seeds=SD.seed_selection(50)
   # SD2 = Single_Discount('NetPHY.txt')
    a=[1,5,10,15,20,25,30,35,40,45,50]
    print(seeds)
    for num in a:
        inf = Evaluation.monte_carlo(SD, seeds, num, 10000)
      #  inf=Evaluation.monte_carlo(SD,seeds,num,10000)
